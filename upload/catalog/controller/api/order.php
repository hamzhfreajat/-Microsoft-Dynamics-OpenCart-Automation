...
class ControllerApiOrder extends Controller {
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
                ...
                $json['order'] = $order_info;
                if ($this->request->post['order_status_id'] == 5 || $this->request->post['order_status_id'] == 7 ){
                    $this -> openCartAxIntegration($products , $this->request->post['order_status_id'] , $order_id);
                }
    ...
    public function openCartAxIntegration($products , $orderStatus , $orderId){
        ...
        $salesID = $soapClient -> CreateSOHeader($headerParam);

        foreach ($products as $product){

            $params = array(
                'salesID'	        =>  $salesID->response,
                '_itemBarCode'		=> $product['model'],
                '_SalesQty'		 	=> (float) $product['quantity'],
                '_SalesPrice'		=> (float) $product['price'],
                '_ItemDiscount'	    => 0.0,
            );
